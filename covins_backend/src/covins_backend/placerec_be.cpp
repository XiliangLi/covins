...
PlaceRecognition::PlaceRecognition(ManagerPtr man, bool perform_pgo)
    : mapmanager_(man), perform_pgo_(perform_pgo), voc_(mapmanager_->GetVoc()),
      mnCovisibilityConsistencyTh(
          covins_params::placerec::cov_consistency_thres) {
  ros::NodeHandle nh, nh_private("~");
  loop_pub_.reset(new coxgraph::mod::LoopClosurePublisher(nh, nh_private));
  //...
}
...
auto PlaceRecognition::ComputeSE3() -> bool {
  ...
  for (size_t i = 0; i < nInitialCandidates; i++) {
    ...
    std::unique_ptr<estd2::DenseMatcher> matcherThreaded(
        new estd2::DenseMatcher(8));
    ...
    if (kf_query_->id_.second == pKF->id_.second &&
        nmatches < covins_params::placerec::matches_thres) {
      vbDiscarded[i] = true;
      continue;
    ...
    vvpMapPointMatches[i] = LandmarkVector(
        kf_query_->keypoints_distorted_.size(), static_cast<LandmarkPtr>(NULL));
    for (Matches::iterator itr = matchesThreaded.begin();
         itr != matchesThreaded.end(); ++itr) {
      const size_t idxA = (*itr).idxA;
      const size_t idxB = (*itr).idxB;
      if (kf_query_->GetLandmark(idxA) && pKF->GetLandmark(idxB)) {
        if (!kf_query_->GetLandmark(idxA)->IsInvalid() &&
            !pKF->GetLandmark(idxB)->IsInvalid()) {
  ...
  for (size_t i = 0; i < nInitialCandidates; ++i) {
    ...
    int numMatches = 0;
    for (LandmarkVector::iterator itr = vvpMapPointMatches[i].begin();
         itr != vvpMapPointMatches[i].end(); ++itr) {
      if ((*itr)) {
        ++numMatches;
      }
    }

    bool foundTransform = se3solver.projectiveAlignment(
        kf_query_, vvpMapPointMatches[i],
        covins_params::placerec::ransac::class_threshold, Twc1); // def: 25
  ...
  KeyframeVector vpLoopConnectedKFs =
      kf_match_->GetConnectedKeyframesByWeight(0);
  vpLoopConnectedKFs.push_back(kf_match_);
  mvpLoopMapPoints.clear();
  for (KeyframeVector::iterator vit = vpLoopConnectedKFs.begin();
  ...
  matcher.SearchByProjection(kf_query_,
                             kf_query_->GetStateExtrinsics().inverse() * mTsw,
                             mvpLoopMapPoints, mvpCurrentMatchedPoints,
                             covins_params::matcher::search_radius_proj);
...
auto PlaceRecognition::ConnectLoop(KeyframePtr kf_query, KeyframePtr kf_match,
                                   TransformType T_smatch_squery,
                                   PoseMap &corrected_poses, MapPtr map)
    -> void {
...
auto PlaceRecognition::CorrectLoop() -> bool {
  ...
  int check_num_map;
  MapPtr map_query = mapmanager_->CheckoutMapExclusiveOrWait(
      kf_query_->id_.second, check_num_map);

  for (auto lc : map_query->GetLoopConstraints()) {
    bool existing_match = false;
    if (lc.kf1 == kf_match_ && lc.kf2 == kf_query_)
      existing_match = true;
    if (lc.kf2 == kf_match_ && lc.kf1 == kf_query_)
      existing_match = true;
    if (!existing_match)
      continue;
    std::cout << "!!! Loop Constraint already exisiting -- skip !!!"
              << std::endl;
    kf_query_->SetErase();
    kf_match_->SetErase();
...
auto PlaceRecognition::DetectLoop() -> bool {
  ...
  if (last_loops_.count(kf_query_->id_.second)) {
    if ((kf_query_->id_.first - last_loops_[kf_query_->id_.second]) <
        covins_params::placerec::consecutive_loop_dist) {
      kf_query_->SetErase();
      return false;
    }
  }
  ...
  vecConsistentGroup
      vCurrentConsistentGroups; // pair <set<KF*>,int> --> int counts consistent
                                // groups found for this group
  ...
  for (size_t i = 0; i < vpCandidateKFs.size(); ++i) {
    ...
    KeyframeSet spCandidateGroup(candidate_connections.begin(),
                                 candidate_connections.end());
    ...
    for (size_t iG = 0; iG < mvConsistentGroups.size(); ++iG) {
      ...
      for (KeyframeSet::iterator sit = spCandidateGroup.begin();
           sit != spCandidateGroup.end(); ++sit) {
        if (sPreviousGroup.count(*sit)) {
          // KF found that is contained in candidate's group and comparison
          // group
          bConsistent = true;
    ...
    if (!bConsistentForSomeGroup) {
      ConsistentGroup cg =
          make_pair(spCandidateGroup,
                    0); // For "ConsistentGroup" the "int" is initialized with 0
      vCurrentConsistentGroups.push_back(cg);
    }
  }
...
auto PlaceRecognition::FuseLandmark(LandmarkPtr lm_target,
                                    LandmarkPtr lm_tofuse, MapPtr map) -> void {
  // should only call this when sure that LMs cannot be modified by other thread
  // currently
  if (!lm_target) {
    return;
  }
  ...
  for (auto i : observations_tofuse) {
    ...
    if (feat_id_target < 0) {
      ...
      lm_tofuse->EraseObservation(
          kf); // we need this, otherwise the landmark will erase some
               // connections from the KF once it is removed from the map
  ...
  if (non_moved_obs < 2) {
    map->EraseLandmark(
        lm_tofuse); // TODO: in case of map fusion, "lm_tofuse" might not be in
                    // "map" and therefore the erasing operation might fail. At
                    // the moment, this is later ironed out by Map::Clean()
  } else
